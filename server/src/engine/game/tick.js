...
const { doLearn } = require("./abilityIndex");
let date = {
	...
	doTick: async function () {
		...
		await allGladiators.forEach(async (gladiator) => {
			...
			if (growth) {
                ...
                console.log(`  -EN/Tick> ${gladiator.name} is Learning`, gladiator.schedule[0][date.weekDay][date.time]);
                await doLearn(gladiator,gladiator.schedule[0][date.weekDay][date.time])
