...
async function clearSaves() {
  ...
  saveDuel
    .deleteMany({ createdAt: { $lt: lessThan } })
    .then(() => {
  ...
  doTick: async function () {
    return new Promise(async (resolve, reject) => {
      ...
      if (date.weekDay == 7) {
        ...
        console.log(
          `  -EN> Saved Gladiators Time: ${new Date() - startOfTick}  :: ${allNonSeedGlad.length}# of glads`,
        );
        ...
        if (date.month === 12 && date.day == 28) {
          ...
          console.log(
            `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms / # of Loops${allGladiators.length} saved:${ditto.usedGlads.length}`,
          );
        } else if (
          ...
          await saveGlads(ditto);
          console.log(
            `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms / # of Loops${allNonSeedGlad.length} saved:${ditto.usedGlads.length}`,
          );
        } else if (date.day == 28) {
        ...
        await allGladiators.forEach(async (gladiator) => {
          ...
          if (
            gladiator.taskSkill[0] == gladiator.schedule[0][weekDay][date.time]
          ) {
            ...
            gladiator.lastGain = growth.map(
              (growth) =>
                `+${growth.amount} ${growth.stat[0] + growth.stat[1] + growth.stat[2] + growth.stat[3]}`,
            );
            gladiator.lastGain.unshift(
              `${gladiator.schedule[0][weekDay][date.time]}`,
            );
            if (growth) {
              ...
              const goldGrowth = growth.find(
                (element) => element.stat === "gold",
              );
              const fameGrowth = growth.find(
                (element) => element.stat === "fame",
              );
              if (goldGrowth) {
          ...
          if (date.time === 8) {
            ...
            if (gladiator.age > 2000000) {
              console.log(
                " FORWARD THINKING AGE, add more memoriy arries or not!",
              );
            }
            gladiator.doLevel();
          }
