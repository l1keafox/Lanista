...
async function saveManyModelMemory(gladArray) {
  ...
  console.log(
    `  -> Added ${createArray.length} / in array: ${gladArray.length}`,
  );
...
async function getMemoryGroup(mainGlad, groupSize) {
  ...
  let MemoryByAge = await Memory.find({
    level: mainGlad[0].level,
  ...
  function getRandomMemorys(memoryGroup, size) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      rtnArray.push(
        memoryGroup[Math.floor(Math.random() * memoryGroup.length)],
      );
