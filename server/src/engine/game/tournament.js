...
async function bestOfThreeTournament(group, name, toRecord) {
  return new Promise(async (resolve, reject) => {
    ...
    do {
      roundCount++;
      console.log(
        "  -TOURN>Start best of three Round ",
        roundCount,
        name,
        group.length,
      );
...
async function roundRobinThenBestOfThree(incomingGroup, tournyName, toRecord) {
  return new Promise(async (resolve, reject) => {
    ...
    console.log(
      "    -EN/TOURNY/ Starting best of three Tournament: Members count:",
      bestOfThreeArry.length,
    );
    let result = await bestOfThreeTournament(
      bestOfThreeArry,
      tournyName + "Final",
      toRecord,
    );
    ...
    resolve({
      ...
      report: { roundRobin: roundRobinReport, bestOfThree: result.report },
    });
  });
}
...
async function singleElimination(group, name, toRecord) {
  return new Promise(async (resolve, reject) => {
    ...
    do {
      ...
      console.log(
        "    -TOURN>Start Round Started with :",
        group.length,
        "Ended with :",
        result.winnerArray.length,
      );
      group = result.winnerArray;
    ...
    console.log(
      "  -EN> Elemintaion :: rount taken to win:",
      roundCount,
      group.length,
    );
...
async function addToRecord2(recObj, maybeGlad, type) {
  ...
  if (maybeGlad.memory) {
    //let notMemory = await Gladiator.findById(maybeGlad.gladiatorId);
    if (!recObj.gladiator[maybeGlad.gladiatorId])
      recObj.gladiator[maybeGlad.gladiatorId] = {};
    if (!recObj.gladiator[maybeGlad.gladiatorId][type])
      recObj.gladiator[maybeGlad.gladiatorId][type] = 0;
    recObj.gladiator[maybeGlad.gladiatorId][type]++;

    // if(!maybeGlad.seed){
    if (!recObj.memory[maybeGlad.id]) recObj.memory[maybeGlad.id] = {};
    if (!recObj.memory[maybeGlad.id][type])
      recObj.memory[maybeGlad.id][type] = 0;
    recObj.memory[maybeGlad.id][type]++;
    //}
  } else {
    if (!recObj.gladiator[maybeGlad.id]) recObj.gladiator[maybeGlad.id] = {};
    if (!recObj.gladiator[maybeGlad.id][type])
      recObj.gladiator[maybeGlad.id][type] = 0;
...
async function doSaveTournament(members, report, saveAs, winner) {
  ...
  let savedTourny = await new saveTournament({
    tournament: JSON.stringify(report),
...
async function nationalTournament(allGladiators) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i in organizeByLvl) {
      ...
      console.log(
        "  -EN/NATIONAL/TOURN>>< Starting National Tournament> FOR LEVEL:",
        mainGlad.level,
        "<",
        tournyName,
        localGroup.length,
      ...
      let result = await roundRobinThenBestOfThree(
        localGroup,
        tournyName,
        toRecordObj,
      );
      ...
      if (result.winner) {
        console.log(
          `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms National Doing National tournament size: ${localGroup.length} age:${mainGlad.age} level: ${mainGlad.level} WINNER: ${result.winner.name}`,
        );
        //await addToRecord(result.winner,"yearWin");
      } else {
        console.log(
          `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms National Doing National tournament size: ${localGroup.length} age:${mainGlad.age} level: ${mainGlad.level} WINNER: NONE?!`,
        );
      }
...
async function quarterTournament(allGladiators) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i in allGladiators) {
      ...
      if (result.winner) {
        console.log(
          `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms ${mainGlad.name} Level:${mainGlad.level} Age:${mainGlad.age} Doing Quarter tournament size: ${localGroup.length} WINNER: ${result.winner.name}`,
        );
        addToRecord2(toRecordObj, result.winner, "quarterWin");
      } else {
        console.log(
          `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms ${mainGlad.name} age:${mainGlad.age} level: ${mainGlad.level} Doing Quarter tournament size: ${localGroup.length} WINNER: NONE?!`,
        );
      }
...
async function regionalTournament(allGladiators) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i in allGladiators) {
      ...
      if (result.winner) {
        console.log(
          `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms ${mainGlad.name} Doing regional tournament size: ${localGroup.length} age:${mainGlad.age} level: ${mainGlad.level} WINNER: ${result.winner.name}`,
        );
        addToRecord2(toRecordObj, result.winner, "monthWin");
      } else {
        console.log(
          `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms ${mainGlad.name} Doing regional tournament size: ${localGroup.length} age:${mainGlad.age} level: ${mainGlad.level} WINNER: NONE?!`,
        );
      }
...
async function localTournament(allGladiators) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i in allGladiators) {
      ...
      console.log(
        `    -EN>Tounry>Tournament Took: ${new Date() - startOfTick}ms Doing Local tournament size: ${localGroup.length} age:${mainGlad.age} level: ${mainGlad.level} ${mainGlad.name} WINNER: ${string}`,
      );
