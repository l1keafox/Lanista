...
const gladiatorSchema = new Schema({
  ...
  age: {
    //This might be how many days passed sense start of training. Will be used later on.
    type: Number,
  },
  ...
  // Physical Stats
  strength: {
    // Muscle - how strong one is.
    type: Number,
  },
  dexterity: {
    //Work with hands - hand eye thing - hitting
    type: Number,
  },
  agility: {
    //Speed - Naturally Quick - physical speed - dodging
    type: Number,
  },
  constitution: {
    //Physical Make up  - resistances to effects
    type: Number,
  },
  vitality: {
    //Energy - state of being strong/active - hit points
    type: Number,
  },
  ...
  wisdom: {
    // Quality of experience, knowledge and good judgement
    type: Number,
  },
  bravery: {
    // Courageous
  ...
  piety: {
    // Duty to respect religious context
    type: Number,
  },
  sensitivity: {
    // Quick to detect changes,signals, or influences
    type: Number,
  },

  // Social Stats
  charisma: {
    // Attractiveness/charm to inspire others
    type: Number,
  },
  luck: {
    // Success or failure brought by chance rather than one’s actions
    type: Number,
  },
  reputation: {
    // Widespread belief of this person.
    type: Number,
  },
  ...
  slots.forEach((slot) => {
    if (this[slot] && getItemEffect(this[slot])) {
      this.abilities = this.abilities.concat(
        getItemEffect(this[slot]).abilities,
      );
    }
  });
  calcauateBase(this);
...
gladiatorSchema.methods.doLevel = async function () {
  ...
  if (this.level == 1) {
    if (this.age > 28) {
      // takes 28 days to lv 2
      this.level++;
    }
  } else if (this.level == 2) {
    if (this.age > 114) {
      // takes 3 months to lv 3
      this.level++;
    }
  } else if (this.level == 3) {
    if (this.age > 228) {
      // takes 6 months to lv 4
      this.level++;
    }
  } else if (this.level == 4) {
    if (this.age > 456) {
      // takes 1 year to lv 5
      this.level++;
    }
