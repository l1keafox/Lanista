...
router.post("/users/createAcct", async (req, res) => {
  ...
  if (userExist) {
    return res
      .status(401)
      .send({ error: "Create Acct failed: username exists" });
  }
  let existUser = await User.findOne({ email });
  if (existUser) {
    return res.status(401).send({ error: "Create Acct failed: email exists" });
  }
  ...
  try {
    ...
    res.status(401).send({ error: "Create Acct failed: email Form" });
  }
});
...
router.post("/users/login", async (req, res) => {
  ...
  try {
    ...
    if (userExist) {
      ...
      return res
        .status(401)
        .send({ error: "Error: No username, Create Account" });
    }
  }
});
