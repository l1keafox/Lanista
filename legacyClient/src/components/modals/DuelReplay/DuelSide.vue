  ...
  <div class="w-[270px] h-[400px] text-slate-900" :class="bgColor">
    ...
    <template v-if="thisGuy">
      ...
      <template v-if="gladiatorData">
        <Character
          :clothes="makeClothes(gladiatorData)"
          v-model="animate"
          :gladName="props.glad.idKey + ''"
          :animation="anime"
          :direction="dir"
          class="h-[16rem] w-[16rem] bottom-16"
        />
      </template>
...
onMounted(async () => {
  const rpnse = await fetch(apiCall.value + `/gladiator/${thisGuy.value.id}`, {
