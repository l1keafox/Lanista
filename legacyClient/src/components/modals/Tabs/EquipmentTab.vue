<template>
  <div>
    <div id="eqTut" class="relative p-6 flex-auto" v-if="gladiatorData">
      ...
      <template v-for="(slot, index) in slots" :key="slot">
        <div class="flex justify-between">
          <div class="flex w-1/2 justify-between">
            ...
            <ItemMouseOver
              :itemId="gladiatorData[slot]"
              :gladData="gladiatorData"
            >
              <template v-if="gladiatorData.value">
                <h2 v-if="gladiatorData.value[slot]">
                  {{ gladiatorData.value[slot] }}
                </h2>
              </template>
              <template v-else>
