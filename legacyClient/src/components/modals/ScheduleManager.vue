<template>
  <BaseModal>
    ...
    <template v-slot:content>
      <div
        id="weekDiv"
        class="flex overflow-x-auto z-20 justify-center items-center bg-slate-400"
      >
      ...
      <div class="bg-slate-700 p-2">
        <div class="flex">
          <div
            v-if="this.gladiatorData && this.gladiatorData.learnSkill[0]"
            id="learnSkiTut"
          >
            <div class="flex">
              <h1>Learn this :</h1>
            ...
            <div class="flex">
              ...
              <p>
                {{
                  this.gladiatorData.progressSkill[
                    this.gladiatorData.learnSkill[0]
                  ]
                }}
              </p>
            </div>
      ...
      <div class="flex flex-wrap">
        ...
        >
          <div
            class="border p-1 border-yellow-800 flex justify-around text-black"
          >
            <h1>{{ training }}</h1>
...
export default {
  ...
  methods: {
    ...
    async doSave() {
      ...
      await fetch(this.apiCall.value + `/gladiator/saveLearning`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: this.gladId,
  ...
  async mounted() {
    this.trainJson = await cacheJson(
    ...
    if (this.gladiatorData.progressSkill) {
      this.gladiatorData.progressSkill = JSON.parse(
        this.gladiatorData.progressSkill,
      );
    }
    ...
    this.showTutorial({
      elementId: "weekDiv",
      message:
        "A day is broken into 8 time frames, each time frame will train skills depending on what you have setup",
      orientation: "bottom",
    });
    this.showTutorial({
      elementId: "learnSkiTut",
      message:
        "Pick the 'Learn' in the schedule, the select the skill you would like to learn in the dropdown. Instead of doing that task, the gladiator will go and learn this skill.",
      orientation: "bottom",
    });
  },
};
