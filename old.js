...
const components = {
	...
	const modals = {
		...
		let timerInterval;
		let tutorialName;
		const apiData = location.protocol === "https:"
			...
			if (isLoggedIn.value && apiData && auth.getUser()) {
				try {
					const rpnse = await fetch(
						apiData + `/owner/${auth.getUser().ownerId}`,
						{
							headers: { "Content-Type": "application/json" },
						}
					);
		...
		provide('card',"h-80 w-56 p-3 m-3 cursor-default select-none flex flex-col bg-slate-700 ");
		provide('smallCard',"h-64 aspect-[5/7] p-3 m-3 cursor-default select-none flex flex-col bg-slate-700 ")
		provide('largeCard',"h-96 aspect-[5/7] p-3 m-3 cursor-default select-none flex flex-col bg-slate-700 " )
		provide('gladiatorCard',"h-[27rem] aspect-[5/7] p-3 m-3 cursor-default select-none flex flex-col bg-slate-700 ")
		provide('cardTitle',"text-xl text-sky-400")
		provide('getOwner',computed(() => ownerData.value) )
		provide('getTime',computed(() => timeData) )
		provide('getUser',computed(() => userData))
		provide('getLogged',computed(() => isLoggedIn))
		provide('apiCall', computed(() => apiData))
