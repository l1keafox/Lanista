  ...
  <div class="flex h-full flex-col">
    ...
    <div v-if="itemData">
      <div v-for="(item,index) in itemData" :key="item" :class="card">
        ...
        <button class="bg-yellow-200 m-2 text-purple-900" @click="buyThing('item',index)">Buy</button>
      </div>
    </div>
    ...
    <div v-if="structData">
      <div v-for="(struct,index) in structData" :key="struct" :class="card">
        ...
        <button class="bg-yellow-200 m-2 text-purple-900" @click="buyThing('struct',index)">Buy</button>
      </div>
    </div>
  </div>
</template>
