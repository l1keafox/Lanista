<template>
  <BaseModal>
    <template v-slot:header>
      ...
      >
        <h3 v-if="tournamentData" class="text-3xl font-semibold">Tournament</h3>
        <div
    ...
    <template v-slot:content>
      <div class="relative p-6 flex-auto overflow-y-auto bg-yellow-200">
        <div v-if="tournamentData">
          <div
            v-for="(fight, index) in tournamentData.tournamentStructure[
              openTab
            ]"
            :key="index"
          >
            <div class="flex justify-between">
              ...
              <button @click="showDuel(fight.saveId, fight[1], fight[2])">
                Duel
              </button>
            </div>
          </div>
